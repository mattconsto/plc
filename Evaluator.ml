...
let rec eval co ce ci env e = flush_all (); match e with
	...
	| TermString (s)           -> eval co ce ci env (
	...
	| TermAssert (a)           -> (if not (equality_test (eval co ce ci env a)) then raise (AssertionFailed a); TermUnit)
	| TermBreak                -> raise LoopBreak
	| TermContinue             -> raise LoopContinue
	| TermExit a               -> raise (Terminated (eval co ce ci env a))
	...
	| TermBind (x, tT, a)     -> let temp = eval co ce ci (bind env x a) a in (ignore (rebind env x temp); temp)
