...
if (Array.length Sys.argv) < 2 then (
	...
	while true do
		...
		Printf.fprintf stdout "<- %s\n" (result_to_int (run stdout stdout stdin (read_line ())))
	done
) else (
	try (
		let load_file f =
...
(* if (Array.length Sys.argv) < 2 then (
	...
	while true do
		Printf.fprintf stdout "-> ";
		Printf.fprintf stdout "<- %s" (ast stdout stdout stdin (read_line ()))
	done
) else (
	try (
		let load_file f =
		  ...
		  close_in ic;
		  s in
		ignore (ast stdout stderr stdin (load_file Sys.argv.(1))))
	with
		| Sys_error m -> Printf.fprintf stderr "%s\n" m
		| a           -> Printf.fprintf stderr "%s\n" (Printexc.to_string a)) *)
